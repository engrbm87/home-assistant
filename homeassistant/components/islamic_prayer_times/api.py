...
class IslamicPrayerApi:
    ...
    def calc_method(self):
        ...
        return CALC_METHODS[self.config_entry.options[CONF_CALC_METHOD]]

    @property
    def school(self):
        """Return the calculation school."""
        return SCHOOLS.index(self.config_entry.options[CONF_SCHOOL])

    @property
    def midnight_mode(self):
        """Return the midnight calculation method."""
        return MIDNIGHT_MODES.index(self.config_entry.options[CONF_MIDNIGHT_MODE])

    @property
    def lat_adj_method(self):
        """Return the latitude adjutsment method."""
        return LAT_ADJ_METHODS.index(self.config_entry.options[CONF_LAT_ADJ_METHOD]) + 1
    ...
    async def async_add_options(self):
        ...
        if not self.config_entry.options:
            ...
            options = {
                CONF_CALC_METHOD: data.pop(CONF_CALC_METHOD, DEFAULT_CALC_METHOD),
                CONF_SCHOOL: data.pop(CONF_SCHOOL, DEFAULT_SCHOOL),
                CONF_MIDNIGHT_MODE: data.pop(CONF_MIDNIGHT_MODE, DEFAULT_MIDNIGHT_MODE),
                CONF_LAT_ADJ_METHOD: data.pop(
